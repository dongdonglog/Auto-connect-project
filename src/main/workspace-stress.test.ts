...
describe('WorkspaceService scale smoke', () => {
  it('indexes and searches a few hundred local notes without losing records', async () => {
    ...
    try {
      service = new WorkspaceService(); await service.create(join(root, 'workspace'), 'Stress')
  ...
  it('serves material relations for a fifty-document workspace within one second', async () => {
    ...
    let service: WorkspaceService | undefined
    try {
      service = new WorkspaceService(); await service.create(join(root, 'workspace'), 'Relations')
      const materials = []
