...
import type { Material, Relation, TopicMap, TopicProposal } from '../../types'
...
export function TopicCanvas({ map, materials, onRefresh, onImportFiles }: { map: TopicMap; materials: Material[]; onRefresh(): Promise<void>; onImportFiles(paths: string[], position: Point): Promise<void> }): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const flowEdges = map.relations.filter((relation) => !relation.archived).map((relation) => ({ id: relation.id, source: relation.sourceMaterialId, target: relation.targetMaterialId })) as Edge[]
...
const proposalKindLabel: Record<string, string> = { create_relation: '创建关系', rename_relation: '重命名关系', set_sequence: '调整顺序', set_card_style: '修改卡片样式', layout: '调整布局', create_workstream: '创建分组' }
...
function ProposalInspector({ proposals, map, onClose, onReview }: { proposals: TopicProposal[]; map: TopicMap; onClose(): void; onReview(proposalId: string, decision: 'accept' | 'archive'): void }): React.ReactElement {
  const materialTitle = (value: unknown): string => map.materials.find((material) => material.id === String(value ?? ''))?.displayTitle ?? map.materials.find((material) => material.id === String(value ?? ''))?.title ?? '未知材料'
  const summary = (proposal: TopicProposal): string => {
    ...
    if (proposal.kind === 'set_sequence') return `${materialTitle(proposal.materialId ?? proposal.payload.materialId)} · 第 ${String(proposal.payload.sequence ?? '')} 位`
    ...
    if (proposal.kind === 'layout') return `${Array.isArray(proposal.payload.positions) ? proposal.payload.positions.length : 0} 张卡片`
    if (proposal.kind === 'rename_relation') return `新名称：${String(proposal.payload.label ?? '')}`
    return proposalKindLabel[proposal.kind] ?? proposal.kind
  }
  return <aside className="whiteboard-inspector proposal-inspector"><header><h3>待审核操作</h3><button className="icon-button" title="关闭" onClick={onClose}>×</button></header>{!proposals.length ? <p className="proposal-empty">当前没有待审核操作。</p> : <div className="proposal-list">{proposals.map((proposal) => <article key={proposal.id}><span>{proposalKindLabel[proposal.kind] ?? proposal.kind}</span><strong>{summary(proposal)}</strong><p>{proposal.reason}</p>{proposal.evidence && <small>依据：{proposal.evidence}</small>}<div><button className="secondary-button" onClick={() => onReview(proposal.id, 'archive')}>忽略</button><button className="primary-button" onClick={() => onReview(proposal.id, 'accept')}>应用</button></div></article>)}</div>}</aside>
}
