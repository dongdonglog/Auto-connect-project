import { ClipboardCheck, LayoutDashboard, Maximize2, Network, Plus, Redo2, Undo2, Upload } from 'lucide-react'
...
const views: Array<{ mode: 'map' | 'flow'; label: string; detail: string; icon: ReactNode }> = [
  ...
  { mode: 'flow', label: '流程视图', detail: '按连接关系从左到右排列', icon: <Network size={19} /> }
]

/** 免模式工具栏：左键框选，拖动卡片移动，Space+左键/中键平移，拖端口连线。 */
export function BoardToolbar({ onAdd, onImport, onLayout, onFit, onUndo, onRedo, onProposals, proposalCount, proposalsOpen, canUndo, canRedo }: { onAdd(): void; onImport(): void; onLayout(): void; onFit(): void; onUndo(): void; onRedo(): void; onProposals(): void; proposalCount: number; proposalsOpen: boolean; canUndo: boolean; canRedo: boolean }): React.ReactElement {
  ...
  return <aside className="whiteboard-tools" aria-label="Topic canvas tools">
    ...
    <button className={proposalsOpen ? 'active proposal-tool' : 'proposal-tool'} aria-label="待审核操作" data-tooltip={`待审核操作${proposalCount ? ` (${proposalCount})` : ''}`} onClick={onProposals}><ClipboardCheck size={19} />{proposalCount > 0 && <span>{proposalCount > 9 ? '9+' : proposalCount}</span>}</button>
    <hr />
