...
export function useTopicConnections(relations: Relation[], onRefresh: () => Promise<void>, onNotice: (message: string) => void): { validate(connection: Pick<Connection, 'source' | 'target'>): string | null; create(connection: Connection): Promise<void> } {
  ...
  const create = useCallback(async (connection: Connection): Promise<void> => {
    ...
    try {
      await ipc.relation.create({ sourceMaterialId: connection.source!, targetMaterialId: connection.target!, label: '关联', relationType: 'related', evidenceText: null, evidenceMaterialId: connection.source!, confidence: null, createdBy: 'manual' })
      onNotice('已创建正式连接。'); await onRefresh()
    } catch (reason) { onNotice(reason instanceof Error ? reason.message : '无法创建连接。') }
  }, [onNotice, onRefresh, validate])
